...
class Block(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_s=3, strides=2, encoding=False, bottleneck=False, decoding=False, relu=True, batchnorm=True):
        ...
        if self.encoding:
            self.l1 = PartialConv2d(in_channels, out_channels, kernel_s, strides, padding="same")
            if self.batchnorm:
            ...
            self.l2 = PartialConv2d(in_channels, out_channels, kernel_s, strides, padding="same")
    ...
    def forward(self, img, mask, concat_img=0, concat_mask=0):
        ...
        if self.decoding:
            ...
            concated_img = torch.cat((upimg_o, concat_img), 2)
            concated_mask torch.cat((upmask_o, concat_mask), 2)
